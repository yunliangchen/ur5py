...
class ContinuousStateRobotiqGripper:
    """
    ...
    def _get_var(self, variable: str):
        ...
        with self.command_lock:
            ...
            try:
                ...
                print("Socket timed out while waiting for response from gripper.")
                self.socket.sendall(cmd.encode(self.ENCODING))
                data = self.socket.recv(1024)
    ...
    def is_active(self):
        ...
        return ContinuousStateRobotiqGripper.GripperStatus(status) == ContinuousStateRobotiqGripper.GripperStatus.ACTIVE
    ...
    def auto_calibrate(self, log: bool = True) -> None:
        ...
        if ContinuousStateRobotiqGripper.ObjectStatus(status) != ContinuousStateRobotiqGripper.ObjectStatus.AT_DEST:
    ...
    def move_and_wait_for_pos(self, position: int, speed: int, force: int) -> Tuple[int, ObjectStatus]:  # noqa
        ...
        while ContinuousStateRobotiqGripper.ObjectStatus(cur_obj) == ContinuousStateRobotiqGripper.ObjectStatus.MOVING:
            cur_obj = self._get_var(self.OBJ)

        # report the actual position and the object status
        final_pos = self._get_var(self.POS)
        final_obj = cur_obj
        return final_pos, ContinuousStateRobotiqGripper.ObjectStatus(final_obj)
    
    def open(self):
        """Opens the gripper."""
        return self.move(0, 255, 255)
    
    def close(self):
        """Closes the gripper."""
        return self.move(255, 255, 255)
    
    def get_current_position_normalized(self):
        """Returns the current position as a float between 0 and 1."""
        # open is 3, closed is 228
        return (self.get_current_position()-3) / 225.0
    
    def convert_normalized_to_unnormalized(self, normalized):
        """Converts a normalized position to an unnormalized position.
        0 ->3 -> 0
        1 -> 228 -> 255
        """
    ...
    def set_pos(self, pos):
        ...
        return self.move(self.convert_normalized_to_unnormalized(pos), 255, 100)